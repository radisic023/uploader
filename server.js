    ...
    if (err) {
        ...
        db.get("SELECT * FROM users WHERE role = 'admin' LIMIT 1", (err, row) => {
            if (err) {
                ...
                db.run("INSERT INTO users (id, username, password, role) VALUES (?, ?, ?, ?)", [adminId, "admin", hashedPass, "admin"], (err) => {
                    if (err) {
...
function addLog(user, action, req) {
    ...
    db.run("INSERT INTO logs (username, action, ip_address, user_agent) VALUES (?, ?, ?, ?)", [user.username, action, ip, userAgent], (err) => {
        if (err) {
...
function createUser({ username, password, role }) {
    ...
    return new Promise((resolve, reject) => {
        db.run("INSERT INTO users (id, username, password, role) VALUES (?, ?, ?, ?)", [id, username, hashedPass, role], (err) => {
            if (err) {
...
app.post("/upload", authenticateToken, (req, res) => {
    ...
    file.mv(filePath, (err) => {
        ...
        db.run(
            `INSERT INTO files (id, encrypted_id, filename, type, tag, password) 
                VALUES (?, ?, ?, ?, ?, ?)`,
...
app.get("/manage", authenticateToken, (req, res) => {
    ...
    const query = req.user.role === "admin" ? "SELECT * FROM files" : "SELECT * FROM files WHERE active = 1";
...
app.post("/deleteFile", authenticateToken, requireAdmin, (req, res) => {
    db.get("SELECT * FROM files WHERE id = ?", [req.body.id], (err, file) => {
        ...
        const filePath = file.tag ? path.join(uploadsDir, "tags", file.tag, file.filename) : path.join(uploadsDir, file.filename);
...
app.get("/file/:encryptedId", (req, res) => {
    try {
        ...
        db.get("SELECT * FROM files WHERE id = ? AND active = 1", [id], (err, file) => {
            ...
            const filePath = path.join(uploadsDir, file.tag ? path.join("tags", file.tag) : "", file.filename);
...
app.post("/editUser", authenticateToken, requireAdmin, (req, res) => {
    ...
    if (password && password.trim() !== "") {
        const hashedPass = hashPassword(password);
        db.run("UPDATE users SET username = ?, password = ?, role = ? WHERE id = ?", [username, hashedPass, role, id], (err) => {
            if (!err) {
